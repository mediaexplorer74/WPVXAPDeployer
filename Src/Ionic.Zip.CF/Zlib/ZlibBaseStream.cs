...
namespace Ionic.Zlib
{
    ...
    internal class ZlibBaseStream : System.IO.Stream
    {
        ...
        private int _ReadAndValidateGzipHeader()
        {
            ...
            while (count > 0)
            {
                read = _stream.Read(header, offset, count);
                // workitem 8501: handle edge case (decompress empty stream)
                if (read == 0)
                    return 0;
                count -= read;
            ...
            if ((header[3] & 0x04) == 0x04)
            {
                ...
                count = 2; // 2-byte length field
                offset = 0;
                while (count > 0)
                {
                    read = _stream.Read(header, offset, count);
                    if (read == 0)
                        throw new ZlibException("Unexpected end-of-file reading XLEN of GZIP header.");
                    count -= read;
                    offset += read;
                    totalBytesRead += read;
                }
                ...
                while (count > 0)
                {
                    read = _stream.Read(extra, offset, count);
                    if (read == 0)
                        throw new ZlibException("Unexpected end-of-file reading extra-field of GZIP header.");
                    count -= read;
                    offset += read;
                    totalBytesRead += read;
                }
